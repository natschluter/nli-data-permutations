...
from utils_shuffle import get_embedding_matrix, get_data, parse_optimizer, get_data_nolabels
...
def get_best_model_on_accuracy():
	epoch_files=[file for file in os.listdir(epochs_folder) if file.startswith(args.dataset+'_weights')]
...
def parse_arguments():	
	...
	parser.add_argument("--subsample", required=False, default='1.0', type=float, help="random subsample rate to filter entailments from training")
	parser.add_argument("--orderbyclass", required=False, default=None, help='order training examples by class, must provide order, ex. CEN')
	parser.add_argument("--sortonlengths", required=False, default=False, action="store_true", help='order training examples by prem+hypo length')
	parser.add_argument("--sortonprem", required=False, default=False, action="store_true", help='order training examples by prem length')
	parser.add_argument("--sortonhypo", required=False, default=False, action="store_true", help='order training examples by prem+hypo length') 
	parser.add_argument("--sortbyconf", required=False, default=False, action="store_true", help='order training examples by confidence') 
	parser.add_argument("--decreasing", required=False, default=False, action="store_true", help='order training examples in decreasing order of length')
	parser.add_argument("--shuffle",  required=False, default=False, action="store_true", help='shuffle training example order')
	parser.add_argument("--backwards",  required=False, default=False, action="store_true", help='reverse training example order')
...
training 	= get_data(args.train, args.limit, skip_neutral=args.binary, subsample=args.subsample, shuffle=args.shuffle, orderbyclass=args.orderbyclass, sortonlengths=args.sortonlengths, sortonprem=args.sortonprem, sortonhypo=args.sortonhypo, decreasing=args.decreasing, backwards=args.backwards, sortbyconf=args.sortbyconf)
...
if not os.path.exists(out_dir):
	os.makedirs(out_dir)
epochs_folder=out_folder+'epochs/'
...
filepath=args.dataset+'_weights.epoch_{epoch:02d}-loss_{val_loss:.4f}-acc_{val_acc:.4f}.hdf5'
if args.save_all_epochs:
...
metrics = model.fit([training[0], training[1]], training[2], batch_size=BATCH_SIZE, epochs=MAX_EPOCHS, validation_data=([validation[0], validation[1]], validation[2]), callbacks=callbacks, verbose=VERBOSE)
...
if args.save_all_epochs:
	...
	if args.test2:
		with open(epochs_folder+args.dataset+'_test2_scores.txt','w') as ofile:
			ofile.write('epoch\tloss\tacc'+header+'\n')	
			ofile.write('\n'.join(sorted(test2s)))
	with open(epochs_folder+args.dataset+'_test_scores.txt','w') as ofile:
		ofile.write('epoch\tloss\tacc'+header+'\n')	
		ofile.write('\n'.join(sorted(tests)))
	with open(epochs_folder+args.dataset+'_dev_scores.txt','w') as ofile:
		ofile.write('epoch\tloss\tacc'+header+'\n')	
		ofile.write('\n'.join(sorted(devs)))
