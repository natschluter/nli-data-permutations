...
class shuffleTrain(keras.callbacks.Callback):
	def on_epoch_end(self):
		print('Shuffling hypotheses')
		np.random.shuffle(training[1])
		print('Shuffling premises')
		np.random.shuffle(training[0])
...
def get_vocabulary():
	print('Loading vocab...')
	dataset_folder='data/' #your data folder here
	text=[]
...
training 	= get_data(args.train, args.limit, skip_neutral=args.binary)
...
shuffle_train_words=shuffleTrain()
callbacks = [early_stop, check_point, shuffle_train_words]
